...
from pymsi.thirdparty.refinery.cab import CabDisk, Cabinet
from pymsi.thirdparty.refinery.structures import StructReader
        ...
        for media in self.medias.values():
            if media._cabinet is None:
                ...
                if not self.package.ole.exists(stream_name):
                    raise ValueError(f"Media file '{media._cabinet[1:]}' not found in the .msi file")
                ...
                if self.package.path.parent not in path.parents:
                    raise ValueError(f"Media file path '{media._cabinet}' attempts to access parent directories")
