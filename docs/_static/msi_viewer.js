...
class MSIViewer {
  ...
  async handleLoadExampleFile() {
    ...
    if (examples.length > 1) {
      selectedExample = await this.showSelectionModal('Select an example to load', examples, 'name');
    ...
    try {
      ...
      const file = new File([blob], selectedExample.filename, { type: blob.type });

      // Reuse the existing file handler which supports ZIP expansion, MSI validation, etc.
